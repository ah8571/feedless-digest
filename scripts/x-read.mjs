 ...
 *
 * Fetches the original tweet with engagement metrics, then finds all
 * replies via conversation_id search and displays them as a tree.
 *
 * Usage:
 *   $env:X_BEARER_TOKEN='...'
 *
 *   node scripts/x-read.mjs 2077859250143990188
 *   node scripts/x-read.mjs https://x.com/user/status/2077859250143990188
 */
...
if (!tweetId) {
  console.error("Usage: node scripts/x-read.mjs <tweet-id-or-url>");
  process.exit(1);
}
...
if (replyRes.ok) {
  ...
  if (replyData.includes?.users) {
    for (const u of replyData.includes.users) {
      replyUsers.set(u.id, u);
    }
  }
}
...
console.log(`  Reply:  npm run x:post -- --reply-to ${tweet.id} "..."`);
console.log(`  Read:   npm run x:read -- ${tweet.id}`);
...
if (replies.length === 0) {
  ...
  // Filter out the original tweet from replies (it's included in conversation)
  ...
  if (actualReplies.length === 0) {
    ...
    // Build tree: direct replies first, then nested
    ...
    for (const reply of actualReplies) {
      ...
      console.log(`     Reply:  npm run x:post -- --reply-to ${reply.id} "..."`);
    }
  }
}
