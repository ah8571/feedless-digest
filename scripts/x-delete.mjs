 ...
 *
 * Requires all 4 OAuth 1.0a credentials. Same auth as x-post.mjs.
 *
 * Env vars:
 ...
 *   Also reads from .env: consumer_key, consumer_key_secret,
 *   access_token, access_token_secret
 *
 * Usage:
 *   npm run x:delete -- <tweet-id>
 *   npm run x:delete -- https://x.com/user/status/123456
 *   npm run x:delete -- --dry-run <tweet-id>
 */
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === '--dry-run') {
      ...
      // Extract ID from URL: https://x.com/user/status/ID
...
if (opts.ids.length === 0) {
  console.log('Usage: npm run x:delete -- <tweet-id-or-url> [<tweet-id-or-url> ...]');
  console.log('       npm run x:delete -- --dry-run <tweet-id>');
  process.exit(1);
}
